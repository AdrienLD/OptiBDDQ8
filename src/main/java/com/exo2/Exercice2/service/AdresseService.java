...
public class AdresseService {
    ...
    @Cacheable(value = "adresse")
    public List<AdresseDto> findAll(Pageable pageable)
    {
        return adresseRepository.findAll(pageable).stream().map(adresseMapper::toDto).toList();
    }

    @Cacheable(value = "adresse", key = "#id")
    public AdresseDto findById(Long id)
    {
        return adresseMapper.toDto(adresseRepository.findById(id).get());
    }

    @Cacheable(value = "adresse", key = "#ville")
